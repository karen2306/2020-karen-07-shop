var Slide = function(arg) {
	...
	this.$slides = [];																			// 생성될 .karen-slide(jQuery객체)를 저장할 배열
...
Slide.prototype.init = function(){
	...
	// stage에 슬라이드를 감싸는 karen-wrapper를 생성 
	this.$wrapper = $('<div class="karen-wrapper karen-'+this.direction+'"></div>').appendTo(this.$container);

	// karen-wrapper에 karen-slide들을 만드는 과정
	for(var i=0, html=''; i<this.slide.length; i++) {
		html  = '<div class="karen-slide">';
		html  = '<div class="karen-slide" style="background-image: url(\'' + this.slide[i] + '\');">';
		html +='<img src="'+this.slide[i]+'" class="w100">';
		if(this.title && this.title[i]) html += this.title[i];
		html += '</div>';

		// hori, vert 형은 karen-wrapper에 실제 태그를 넣어놔야 되므로...
		// 1. 실제태그도 붙이고
	...
	// hori, vert type은 0번 태그를 복제하여 마지막에 붙여 놓는다. 
	if(this.direction === 'hori' || this.direction === 'vert') {
		this.$slides.push($(this.$slides[0].clone()).appendTo(this.$wrapper));
		if(this.title) this.title.push(this.title[0]);					// 타이틀 0번을 마지막에 복제하는 과정
	}

	// fade type은 0번 슬라이드를 karen-wrapper에 붙인다. 
	...
	if(this.btnUse) {
	this.$btnPrev = $('<div class="karen-btn karen-prev">《</div>').appendTo(this.$container);
	this.$btnNext = $('<div class="karen-btn karen-next">》</div>').appendTo(this.$container);
	// click(cb)안에서의 콜백함수(cb)는 this가 클릭메서드의 객체이므로 bind(this)를 통해서 Slide자신을 보낸다.  
	...
	// pager 생성 및 이벤트 적용 
	if(this.pagerUse) {
		// karen-pagers를 this.$container에 생성한다. 
		this.$pagers = $('<div class="karen-pagers"></div>').appendTo(this.$container);

		// 각각의 pager를 생성하고 클릭 이벤트를 적용한다. 
		for(var i in this.slide) {
			$('<div class="karen-pager"></div>').appendTo(this.$pagers).click(this.onPagerClick.bind(this));
		}
		//0번 idx에 .active 클래스를 적용한다. 
		this.$pagers.find(".karen-pager").eq(0).addClass("active");
	}
	...
	this.$container.append(html);
		...
		$(window).resize(function() {
			...
			this.$wrapper.find('.karen-slide').css({
...
Slide.prototype.onMouseOver = function() {
	if(this.autoUse) clearInterval(this.interval);
}
...
// karen-wrapper의 animation
Slide.prototype.ani = function() {
	if(this.pagerUse) {
		this.$pagers.find(".karen-pager").eq(this.now).addClass("active").siblings().removeClass("active");
		if(this.direction === 'hori' || this.direction === 'vert') {
			if(this.now == this.last) this.$pagers.find(".karen-pager").eq(0).addClass("active").siblings().removeClass("active");
		}
	}
